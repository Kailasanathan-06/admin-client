...
function renderClients() {
    ...
    grid.innerHTML = filtered.map(c => {
        ...
        return `<div class="col-xl-3 col-lg-4 col-md-6 mb-3 client-card-wrapper">
            <div class="client-card p-3 ${isAdmin ? 'border-primary' : ''} ${isSelected ? 'border-success' : ''}" style="cursor:pointer;">
                <div class="d-flex justify-content-between align-items-start mb-2">
                    ...
                    <div class="d-flex align-items-center gap-2 ms-2">
                        ${c.approved && !isAdmin ? `<button class="btn btn-sm btn-outline-info" onclick="event.stopPropagation();scanClient('${c.registration_key}')" title="Request scan"><i class="bi bi-play-fill"></i></button>` : ''}
                        ${!c.approved ? `<button class="btn btn-sm btn-outline-success" onclick="event.stopPropagation();approveClient('${c.registration_key}')" title="Approve"><i class="bi bi-check-lg"></i></button>` : ''}
                        <input type="checkbox" class="form-check-input" ${isSelected ? 'checked' : ''} onclick="event.stopPropagation();toggleSelection('${c.registration_key}')">
                        <span class="status-dot ${c.is_stale ? 'offline' : c.status === 'online' ? 'online' : c.status === 'pending' ? 'pending' : 'offline'}"></span>
                    </div>
                </div>
                <div class="small text-secondary">
                    <div>${c.platform || 'Unknown'}${c.cpu_model ? ' | ' + escapeHtml(c.cpu_model) : ''}</div>
                    <div>Last seen: ${timeAgo(c.last_seen)}${c.is_stale ? ' (stale)' : ''}</div>
...
function exportCSV() {
    ...
    const headers = ['Hostname', 'Key', 'Platform', 'Group', 'Tags', 'Status', 'Last Seen', 'Cost', 'Vendor', 'Notes'];
    const rows = clients.map(c => [
        c.hostname || '', c.registration_key, c.platform || '', c.group_name || '', (c.tags_list || []).join('; '),
        c.status || '', c.last_seen || '', c.purchase_cost || '', c.vendor_name || '', (c.notes || '').replace(/"/g, '""')
    ]);
